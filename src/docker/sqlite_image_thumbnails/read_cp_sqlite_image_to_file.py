...
def extract_image_from_sqlite(db_path, column_name, output_folder):
    ...
    for idx, row in enumerate(rows):
        ...
        if image.mode == 'RGB':
            image_np = image_np.reshape((image.size[1], image.size[0], 3))  # Height, Width, Channels
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Extract images from a SQLite database.')
    parser.add_argument('db_path', type=str, help='Path to the SQLite database file.')
    parser.add_argument('column_name', type=str, help='Column name containing the image data.')
    parser.add_argument('output_folder', type=str, help='Directory to save the extracted images.')
